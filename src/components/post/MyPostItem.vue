...
<template>
    <li class="collection-item avatar">
        ...
        <router-link :to="{ name: 'post', params: { id: post.uuid, lang: getLang(), route: 'my-post-list' } }"
        ...
        <router-link :to="{ name: 'amend', params: { id: post.uuid, lang: getLang() } }"
            :title="$t('message.edit')" class="secondary-content edit-my-post" style="right: 50px;">
            <i class="material-icons dp48">edit</i>
        </router-link>

        <a href="#modal_post_delete" class="secondary-content modal-trigger" :id="post.uuid" :title="$t('message.delete')"
            @click="$emit('update:modelValue', post.uuid)">
